...
const slackClientRegistry = new Map();
let baseLogger = null;
const jobs = new Map();

export const registerSlackClient = (botName, client) => {
...
const buildPrompt = (agent, message) =>
  [
...
const sendToSlack = async (destination, text, logger, agentName) => {
  if (!destination?.channel) return;
  const botName =
    destination.bot ??
    (agentName && slackClientRegistry.has(agentName) ? agentName : [...slackClientRegistry.keys()][0]);
  const client = slackClientRegistry.get(botName);
  if (!client) {
    logger?.warn({ botName, available: [...slackClientRegistry.keys()] }, 'no slack client registered for job destination');
    return;
  }
  try {
    ...
    logger?.info({ botName, channel: destination.channel, ts: res.ts, ok: res.ok }, 'slack message posted');
  } catch (err) {
    logger?.error({ err, botName, channel: destination.channel }, 'slack postMessage failed');
    throw err;
  }
};
...
const runJob = async (job) => {
  ...
  if (job.running) {
    logger?.warn({ expr: job.expr }, 'cron job firing skipped — previous run still in progress');
    return;
  }
  job.running = true;
  ...
  try {
    ...
    if (!response.trim()) {
      logger?.warn({ durationMs: result?.durationMs }, 'cron job returned empty response — skipping slack post');
      job.lastRun = { at: new Date().toISOString(), ok: false, error: 'empty response from agent', durationMs: result?.durationMs };
      await persist();
      if (job.destination?.type === 'slack') {
        await sendToSlack(job.destination, `:warning: job *${job.name ?? job.id}* terminou sem texto.`, logger).catch(() => {});
      }
      return;
    }
    job.lastRun = { at: new Date().toISOString(), ok: true, durationMs: result?.durationMs };
    await persist();
    ...
    job.lastRun = { at: new Date().toISOString(), ok: false, error: err.message };
    await persist();

    if (job.destination?.type === 'slack') {
      await sendToSlack(job.destination, `:warning: job *${job.id}* falhou: ${err.message}`, logger, job.agent).catch(() => {});
    }
  } finally {
    job.running = false;
  }
...
const persist = async () => {
  const data = [...jobs.values()].map(({ task, running, ...j }) => j);
  await fs.mkdir(path.dirname(JOBS_FILE), { recursive: true });
  await fs.writeFile(JOBS_FILE, JSON.stringify(data, null, 2));
};

export const addJob = async ({ expr, agent, message, destination, name, catchUp, catchUpWindowMs }) => {
...
const findMissedFiring = (job, now = new Date()) => {
  ...
  try {
    const interval = cronParser.parseExpression(job.expr, { currentDate: startFrom, endDate: now });
    let last = null;
    while (true) {
      try {
        last = interval.next();
...
export const listJobs = () => [...jobs.values()].map(serialize);
...
const serialize = ({ task, running, ...j }) => ({ ...j, running: !!running });
